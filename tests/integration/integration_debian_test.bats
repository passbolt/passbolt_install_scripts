...
@test "not root" {
  run bash dist/debian/passbolt_debian_installer.sh
  [ "$status" -eq 1 ]
}

@test "invalid parameters" {
  run sudo bash dist/debian/passbolt_debian_installer.sh -r -d test
  [ "$status" -eq 1 ]
}

@test "local installation non-interactive non SSL with haveged" {
  echo "curl" >> dist/debian/conf/packages.txt
  run sudo bash dist/debian/passbolt_debian_installer.sh -P root_pass -p passbolt -d passbolt -u passbolt -H passbolt.local -n -e
  [ "$status" -eq 0 ]
}
...
@test "it has php-fpm running" {
  run sudo systemctl is-active php7.0-fpm
  [ "$status" -eq 0 ]
}

@test "it has cron job set up" {
  sudo grep "* * * * * /var/www/passbolt/bin/cake EmailQueue.sender" /var/spool/cron/crontabs/www-data
}

@test "gpg keyring is in place" {
  run sudo stat /home/www-data/.gnupg
  [ "$status" -eq 0 ]
}
